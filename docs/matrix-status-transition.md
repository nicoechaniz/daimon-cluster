...
3. With the reviewed interpreter, stage the reviewed request:
   ...
   validates the original V2 external pair against the legacy runtime checkpoint.
...
- No portable snapshot restore, post-effect runtime downgrade, custody rewind,
  ...
  migration. Only existing V2 legacy status pairs and V3 successors are supported.
...
- This helper imports private Matrix upgrade and public verification primitives;
  ...
  unchanged Cluster pin does not yet provide that successor.
...
qualification. The parent owns real installed-pair and composed receiver tests;
