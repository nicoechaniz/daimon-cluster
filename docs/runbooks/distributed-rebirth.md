...
recovery transfer and retry with the same UUID after interruption. Recovery
restore rejects general portable snapshots: the transfer must contain exactly
the public runtime bundle and canonical ledger, with no custody bytes.
...
This reproducibility is not physical-host uniqueness. Host locks and journals
are local, so a copied package could otherwise start the same embodiment on
two isolated hosts. A release or live ceremony must therefore use a shared
hosting-admission authority that fences the exact embodiment/incarnation and
prove a concurrent second launch is rejected. Until that gate exists, these
commands are disposable qualification tools and must not be described as an
exactly-one physical embodiment guarantee.

The current recovery-custody command is likewise an offline single-store
fixture: one process temporarily opens all threshold seeds. Live custody needs
independent holder signature artifacts plus an aggregator that never receives
private seeds. Synthetic process separation is not distributed quorum proof.
