...
'''
    ...
    # Reconstruct the historical external status pair from synthetic custody only.
    ...
    status = next(r for r in bundle["capabilities"] if ":status:" in r["secret_slot"])
    ...
    upgrade._copy({"client.json": canonical_bytes(dict(schema="dm.local.client-config/v2", capability=status["descriptor"], expected_server=bundle["local_origin"])), "capability.key": custody.secrets[status["secret_slot"]]}, target)
