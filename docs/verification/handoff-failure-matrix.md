...
| 10 | after CAS, target create fails | adapter.create_instance raises | both parked; exact monotonic successor retained, source attachment untouched | tests/test_handoff_production.py::test_production_transfer_commits_cas_before_adapter_mutation |
| 11 | after CAS, before start (start fails) | adapter.start raises after fence acquired | both parked; committed successor is retained and never rolled back | tests/test_handoff_failures.py::test_transfer_start_failure_preserves_safe_fence_successor |
...
re-verifies the Ed25519 manifest and authority receipt, durably prepares the
