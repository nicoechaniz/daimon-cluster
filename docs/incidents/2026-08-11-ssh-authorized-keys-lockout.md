...
During a second-backup-target rehearsal, an operator automation replaced
`/home/debian/.ssh/authorized_keys` with one temporary restricted mirror key
instead of appending it to the existing administrative key. New SSH sessions
using the normal key were rejected until the original key was restored through
the already-running daimonmatrix agent.
