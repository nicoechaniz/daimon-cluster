...
from clusterctl.matrix_fencing.fences import SyntheticResourceFenceStore
...
def test_installed_service_module_invocation_works():
    repository = Path(__file__).resolve().parents[1]
    environment = os.environ.copy()
    environment["VIRTUAL_ENV"] = sys.prefix

    result = subprocess.run(
        [sys.executable, "-m", "clusterctl.matrix_host", "--help"],
        cwd=repository,
        env=environment,
        text=True,
        capture_output=True,
