...
def test_application_startup_failure_releases_real_fixture_admission(monkeypatch):
    # Reuse the existing disposable signed-runtime/admission fixtures. This
    # exercises the actual child host and pin guard, not the injected lifecycle.
    import tempfile
    import time
    from pathlib import Path
    import test_rebirth as fixtures
    from clusterctl import rebirth, rebirth_host

    real_popen = rebirth_host.subprocess.Popen
    children = []

    def configured_child(argv, **kwargs):
        process = real_popen(
            [*argv, "--messaging-application", "/missing-synthetic-application"],
            **kwargs,
        )
        children.append(process)
        return process

    with tempfile.TemporaryDirectory(prefix="dmc-msg-") as name:
        try:
            fixture, state, values = fixtures._install(
                Path(name),
                ceremony_now_ms=time.time_ns() // 1_000_000,
            )
            installed = rebirth.install_rebirth_package(**values)
            fixtures._ensure_production_fences(state, installed["embodiment_id"])
            identity = rebirth_host._installed_identity(
                state, installed["embodiment_id"]
            )
            observer = rebirth_host._configured_admission_client(state, identity)
            monkeypatch.setattr(rebirth_host.subprocess, "Popen", configured_child)
            with pytest.raises(
                rebirth_host.RebirthHostError,
                match="matrix_messaging_application_rejected",
            ):
                rebirth_host.launch_rebirth_host(
                    state,
                    installed["embodiment_id"],
                    fixtures._descriptor(fixture["password"]),
                )
            ...
            fixtures._close_disposable_admission_servers()
