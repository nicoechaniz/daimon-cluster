...
class ResourceFenceStore:
    """Resource fence facade with explicit compatibility and production modes.

    The default remains the historical file backend so existing offline
    fixtures and v0 handoff tests keep working.  Production callers must opt
    into :meth:`production`; that path refuses synthetic signers and delegates
    every operation to one transactional SQLite authority.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if production:
            ...
            self._backend = ProductionFenceStore(
                state_dir,
                signer=signer,
                key_id=key_id,
